...
$sql_check_admin = "SELECT * FROM users WHERE username = 'admin' LIMIT 1";
...
if (mysqli_num_rows($result_order_items) == 0) {
    ...
    if (mysqli_num_rows($result_orders) > 0 && mysqli_num_rows($result_products) > 0) {
        ...
        if (count($order_ids) > 0 && count($product_ids) > 0) {
            ...
            $sql_insert_order_items = "INSERT INTO order_details (order_id, product_id, quantity, price, subtotal) VALUES 
