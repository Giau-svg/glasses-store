...
$sql = "SELECT u.*, r.role_name FROM users u JOIN roles r ON u.role_id = r.role_id $where_clause ORDER BY u.user_id DESC LIMIT $offset, $records_per_page";
...
if (isset($_GET['delete']) && isset($_GET['id'])) {
    ...
    $check_sql = "SELECT user_id, role_id FROM users WHERE user_id = $id";
...
$role_count_sql = "SELECT r.role_name, COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.role_id WHERE r.role_name != 'customer' GROUP BY r.role_name";
$role_count_result = mysqli_query($connect, $role_count_sql);
                                        ...
                                        <?php if (mysqli_num_rows($result) > 0): ?>
                                            <?php while ($each = mysqli_fetch_assoc($result)): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php
                                                            $status = isset($each['status']) ? $each['status'] : 'active';
                                                            echo $status === 'locked'
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="?delete=1&id=<?php echo $each['user_id']; ?>" class="btn btn-danger btn-sm mb-1" onclick="return confirm('Bạn có chắc chắn muốn xóa tài khoản này?')">
